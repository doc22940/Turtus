...
module.exports = class ClientManager extends EventEmitter {
  ...
  async init(){
    /** @type {Rooms} */
    this.rooms = new Map()
    const path = this.storage._path('rooms')
    const rooms = await this.storage.get(path)
    if(!rooms) return
    // Todo: construct rooms map from database if room is "online"
  }
